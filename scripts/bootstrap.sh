...
ssh -tt diogo@k3s-master "cd ~/homelab; sudo k0s install controller --enable-worker -c ./k0s.yaml; sudo k0s start; sleep 5; sudo k0s status; sudo k0s kubeconfig admin > ~/.kube/config"

scp -q diogo@k3s-master:/home/diogo/.kube/config ~/.kube/config || true
